...
import { buildLogFilter, buildSortObject, buildSeverityAggregationPipeline } from '../utils/queryBuilder.util';
...
export const getLogs = async (query: LogQueryParams) => {
  ...
  if (query.sortBy === 'severity') {
    const pipeline = buildSeverityAggregationPipeline(filter, query.sortOrder, skip, limit);
